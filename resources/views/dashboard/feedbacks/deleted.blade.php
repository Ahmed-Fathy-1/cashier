            ...
            <div class="p-4 sm:p-5">
                <table class="min-w-full divide-y divide-slate-200 dark:divide-navy-500">
                    <thead class="bg-slate-50 dark:bg-navy-600">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider dark:text-navy-200">ID</th>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider dark:text-navy-200">Feedback</th>
                    ...
                    <tbody class="bg-white divide-y divide-slate-200 dark:bg-navy-700 dark:divide-navy-600">
                        @foreach ($feedbacks as $feedback)
                            <tr>
                                <td class="px-6 py-4 text-sm font-medium text-slate-900 dark:text-navy-50">{{ $feedback->id }}</td>
                                ...
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-navy-100">{{ $feedback->content }}</td>
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-navy-100 flex space-x-2">
                                    ...
                                    <form action="{{ route('feedbacks.restore', $feedback->id) }}" method="POST">
                                        ...
                                        <button type="submit" class="text-primary hover:text-primary-focus">
                                            <i class="fas fa-undo"></i>
                                        </button>
                                    </form>
                                
                                    <!-- Permanent Delete Icon -->
                                    <form action="{{ route('feedbacks.permdelete', $feedback->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to permanently delete this feedback?');">
                                        @csrf
                                        @method('DELETE')
