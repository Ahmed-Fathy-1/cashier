                        ...
                        @foreach ($feedbacks as $feedback)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-navy-100">
                                    <form action="{{ route('feedbacks.permdelete', $feedback->id) }}" method="POST"
                                        onsubmit="return confirm('Are you sure you want to delete this feedback?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="btn bg-error text-white hover:bg-error-focus focus:bg-error-focus dark:bg-error dark:hover:bg-error-focus dark:focus:bg-error-focus">
                                            Permanent Delete
                                        </button>
